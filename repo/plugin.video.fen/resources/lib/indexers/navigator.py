...
class Navigator:
    ...
    def tips(self):
        ...
        for tip in files:
            ...
            self._add_dir({'mode': 'show_text', 'text_file': xbmc.translatePath(os.path.join(tips_location, tip)), 'exclude_external': 'true', 'heading': 'Fen Tips', 'foldername': '[B]Fen Tips %s[/B]' % tip_name, 'list_name': 'Fen Tips [B]%s[/B]' % tip_name}, '[B]TIPS : [/B] %s' % tip_name, iconImage='faq8.png')
        self._end_directory()
