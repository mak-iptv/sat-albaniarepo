...
def trakt_collection_widgets(db_type, param1, param2):
    # param1 = the type of list to be returned (from 'new_page' param), param2 and param3 is currently not used
    from modules.nav_utils import paginate_list
...
def get_trakt_my_calendar():
    ...
    except ImportError: unicodedata = None
    ...
    def _process(item):
        meta = tvshow_meta('tmdb_id', item['tmdb_id'], meta_user_info, unicodedata)
        result.append(build_episode({"season": item['season'], "episode": item['episode'], "meta": meta, "action": "trakt_calendar",
                                     "include_unaired": True, "first_aired": item['first_aired'], 'trakt_calendar': trakt_calendar}, watched_info, use_trakt, meta_user_info))
    params = dict(parse_qsl(sys.argv[2].replace('?','')))
    ...
    data = [{'tmdb_id': get_trakt_tvshow_id(i['show']['ids']), 'season': i['episode']['season'], 'episode': i['episode']['number'], 'first_aired': i['first_aired']} for i in data if i['episode']['season'] > 0]
    data = [i for i in data if i['tmdb_id'] != None]
    data = [i for n, i in enumerate(data) if i not in data[n + 1:]] # remove duplicates
    if not trakt_calendar:
        try: limit = int(__addon__.getSetting('trakt_widget_limit'))
        except: limit = 20
    ...
    for item in data: threads.append(Thread(target=_process, args=(item,)))
    [i.start() for i in threads]
    [i.join() for i in threads]
    r = [i for i in result if i is not None]
    r = sorted(r, key=lambda k: k['first_aired'], reverse=True)
    ...
    if settings.calendar_focus_today() and trakt_calendar:
        try: index = max([i for i, x in enumerate([i['label'] for i in r]) if x.startswith('[TODAY]')])
        except: index = None
        if index:
