...
class Subtitles(xbmc.Player):
    ...
    def get(self, query, imdb_id, season, episode):
        ...
        xbmc.sleep(2500)
        language = self.language_dict[self.settings_language1]
        language2 = self.language_dict[self.settings_language2]
        try: available_sub_language = xbmc.Player().getSubtitles()
        except: available_sub_language = ''
        if available_sub_language in (language, language2):
            if self.auto_enable == 'true': xbmc.Player().showSubtitles(True)
            return _notification('Local Subtitles Found')
        import os
        ...
        if not result or len(result) == 0:
            language = self.language_dict[self.settings_language2]
            if language == None: return _notification('No Subtitles Found')
        ...
        if self.subs_action == 'Select':
            ...
            xbmc.Player().pause()
            choices = [i for i in result if i['SubLanguageID'] == self.language_dict[self.settings_language1] and i['SubSumCD'] == '1']
            choices += [i for i in result if i['SubLanguageID'] == self.language_dict[self.settings_language2] and i['SubSumCD'] == '1']
            ...
            xbmc.Player().pause()
            if not chosen_sub: return _notification('No Subtitles Selected', _time=1500)
        else:
